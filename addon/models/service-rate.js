  ...
  setServiceRateFees(fees = []) {
    ...
    for (let i = 0; i < fees.length; i++) {
      ...
      if (isModel(rateFee)) {
        ...
        serviceRateFees.pushObject(
          store.createRecord('service-rate-fee', rateFee)
        );
      }
    }
  ...
  setServiceRateParcelFees(fees = []) {
    ...
    for (let i = 0; i < fees.length; i++) {
      ...
      if (isModel(parcelFee)) {
        ...
        serviceRateParcelFees.pushObject(
          store.createRecord('service-rate-parcel-fee', parcelFee)
        );
      }
    }
