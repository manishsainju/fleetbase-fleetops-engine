...
function positionToGeographic(position) {
  ...
  var y = position[1];
  return [
    radToDeg(x / EarthRadius) -
      Math.floor((radToDeg(x / EarthRadius) + 180) / 360) * 360,
    radToDeg(Math.PI / 2 - 2 * Math.atan(Math.exp((-1.0 * y) / EarthRadius))),
  ];
}
...
function positionToMercator(position) {
  ...
  return [
    degToRad(lng) * EarthRadius,
    (EarthRadius / 2.0) *
      Math.log(
        (1.0 + Math.sin(degToRad(lat))) / (1.0 - Math.sin(degToRad(lat)))
      ),
  ];
}
...
function applyConverter(geojson, converter, noCrs) {
  if (geojson.type === 'Point') {
    ...
    for (var f = 0; f < geojson.features.length; f++) {
      geojson.features[f] = applyConverter(
        geojson.features[f],
        converter,
        true
      );
    ...
    for (var g = 0; g < geojson.geometries.length; g++) {
      geojson.geometries[g] = applyConverter(
        geojson.geometries[g],
        converter,
        true
      );
    }
...
export default class Circle extends GeoJson {
  ...
  static createCircle(center, radius, interpolate) {
    ...
    for (var i = 1; i <= steps; i++) {
      ...
      polygon.coordinates[0].push([
        mercatorPosition[0] + radius * Math.cos(radians),
        mercatorPosition[1] + radius * Math.sin(radians),
      ]);
    }
  ...
  recalculate() {
    this.geometry = Circle.createCircle(
