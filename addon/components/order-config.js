...
export default class OrderConfigComponent extends Component {
  ...
  @computed('selected', 'configurations.[]') get orderConfig() {
    return this.configurations.find(
      (config) => config.namespace === this.selected
    );
  }
      ...
      confirm: (modal) => {
        ...
        if (!name) {
          ...
          return this.notifications.warning(
            'Order configuration requires a name'
          );
        }
        ...
        return this.fetch
          ...
          .then((newConfig) => {
            const newConfigExtension = this.fetch.jsonToModel(
              newConfig,
              'extension'
            );
            ...
            this.notifications.success(
              'New order config created successfully.'
            );
  ...
  @action saveConfig() {
    ...
    this.fetch
      ...
      .then(() => {
        this.notifications.success(
          `${orderConfig.name} order configuration saved.`
        );
        this.isSaving = false;
      })
      .catch((error) => {
  ...
  @action deleteExtension(extension) {
    const uninstall = isModel(extension)
      ? extension
      : this.fetch.jsonToModel(extension, 'extension');
      ...
      confirm: (modal, done) => {
        ...
        extension
          ...
          .then(() => {
            ...
            this.notifications.success(
              `Extension '${extensionName}' uninstalled.`
            );
            this.configurations.removeObject(extension);
            this.selected = undefined;
            done();
