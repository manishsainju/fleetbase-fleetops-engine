...
export default class OrderConfigActivityFlowEditorComponent extends Component {
  ...
  @computed(
    'orderConfig',
    'flow',
    'orderConfig.meta.fields.[]',
    'dynamicMetaFields.[]'
  )
  get configFields() {
    const { flow, dynamicMetaFields } = this;
    const defaultLogicFields = [
      'status',
      'type',
      'internal_id',
      'id',
    ...
    const metaFields = Array.from(this.orderConfig.meta.fields ?? []).map(
      (metaField) => `meta.${metaField.key}`
    );
      ...
      confirm: async (modal) => {
        ...
        const status =
          this.activeTab === 'order'
            ? format(statusName)
            : `${this.activeTab}|${format(statusName)}`;
  ...
  @action moveStatus(el, container) {
    ...
    if (['created', 'dispatched', 'completed'].includes(format(status))) {
      this.notifications.warning(
        `The '${format(
          status
        )}' must be in this order of sequence, unable to shift.`
      );
    ...
    if (format(status) !== 'created' && sequence === 0) {
      this.notifications.warning(
        `The created status must always be the first sequence of an order flow.`
      );
      this.drake.cancel(true);
      return;
    }
  ...
  @action updateActivityLogic(status, event, conditionIndex, index, value) {
    ...
    const eventIndex = this.flow[status]['events'].findIndex(
      (e) => e.code === event.code
    );
    ...
    set(
      this.flow,
      `${status}.events.${eventIndex}.if.${conditionIndex}.${index}`,
      value
    );
  ...
  @action addLogicCondition(status, event) {
    ...
    const condition = ['if', '=', ''];
    const index = this.flow[status]['events'].findIndex(
      (e) => e.code === event.code
    );
