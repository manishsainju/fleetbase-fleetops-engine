...
export default class EventTimelineComponent extends Component {
  ...
  timelinePosition(tl) {
    const position =
      tl.items.length > 2 ? tl.items[currentIndex].offsetLeft : 0;
  ...
  setUpVerticalTimeline(tl) {
    ...
    tl.items.forEach((item, i) => {
      ...
      if (
        !this.isElementInViewport(item, tl.settings.verticalTrigger) &&
        i > 0
      ) {
    ...
    if (
      (tlContainer.length && tlContainer.css('overflow-y') == 'scroll') ||
      tlContainer.css('overflow-y') == 'auto'
    ) {
