...
export default class EventTimelineComponent extends Component {
    /**
     * Default event timeline settings
     *
     * @var {Object}
     */
    defaultSettings = {
    ...
    /**
     * Creates a timeline object from the component root element
     *
     * @param {HTMLElement} element
     * @return
     */
    createTimeline(element, defaultSettings = {}) {
        const timelineName = element.id ? `#${element.id}` : `.${element.className}`;
        ...
        try {
            wrap = element.querySelector('.timeline__wrap');
            if (!wrap) {
        ...
        if (settings.startIndex >= items.length - settings.visibleItems) {
            console.warn(
                `${warningLabel} The 'startIndex' setting must be between 0 and ${items.length - settings.visibleItems} for this timeline. The value of ${
    ...
    isElementInViewport(el, triggerPosition) {
    ...
    timelinePosition(tl) {
        const position = tl.items.length > 2 ? tl.items[currentIndex].offsetLeft : 0;
    ...
    setUpVerticalTimeline(tl) {
        ...
        tl.items.forEach((item, i) => {
            ...
            if (!this.isElementInViewport(item, tl.settings.verticalTrigger) && i > 0) {
        ...
        window.addEventListener('scroll', () => {
            tl.items.forEach((item) => {
                if (this.isElementInViewport(item, tl.settings.verticalTrigger)) {
                    item.classList.add('fadeIn');
                }
            });
        });
        // also bring element into view it the container element is scrollable
        const tlContainer = window.$(tl.timelineEl).parent();
        if ((tlContainer.length && tlContainer.css('overflow-y') == 'scroll') || tlContainer.css('overflow-y') == 'auto') {
            const tlContainerDOM = tlContainer.get(0);
            // console.log(tlContainerDOM);
            tlContainerDOM.addEventListener('scroll', () => {
                tl.items.forEach((item) => {
                    if (this.isElementInViewport(item, tl.settings.verticalTrigger)) {
                        item.classList.add('fadeIn');
                    }
