...
export default class ServiceAreasService extends Service {
  ...
  @action addToCache(serviceArea) {
    ...
    if (isArray(serviceAreas)) {
      this.appCache.setEmberData('serviceAreas', [
        ...serviceAreas,
        serviceArea,
      ]);
    } else {
  ...
  @action createServiceArea() {
    ...
    this.notifications.info(
      'Use drawing controls to the right to draw a service area, complete point connections to save service area.',
      {
      ...
      confirm: (modal) => {
        ...
        return record.save().then((record) => {
          this.notifications.success(
            `New ${selectedLayerType} '${record.name}' saved.`
          );
    ...
    this.modalsManager.show('modals/service-area-form', {
      ...
      confirm: (modal) => {
        ...
        return serviceArea.save().then((serviceArea) => {
          this.notifications.success(
            `New service area '${serviceArea.name}' saved.`
          );
  ...
  @action createZone(serviceArea) {
    ...
    this.notifications.info(
      'Use the drawing controls to the right to draw a zone within the service area, complete point connections to save the zone.',
      {
        clearDuration: 1000 * 9,
      }
    );
    ...
    this.modalsManager.show('modals/zone-form', {
      ...
      confirm: (modal) => {
        ...
        return zone.save().then(() => {
          this.notifications.success(
            `New zone '${zone.name}' added to '${serviceArea.name}' service area.`
          );
