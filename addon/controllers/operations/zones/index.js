...
export default class OperationsZonesIndexController extends Controller {
  ...
  startZoneEditing(serviceArea) {
    ...
    this.notifications.info(
      'Use the draw controls to the left to define, edit, or delete zones within your service area, your zone is not limited to the bounds of your service area.'
    );
  }
  ...
  makeZoneEditable(zone, { target }) {
    if (
      this.editableZones.includes(target) ||
      !this.activeFeatureGroup ||
      this.activeFeatureGroup.hasLayer(target)
    ) {
      return;
    }
  ...
  createZone(attrs = {}) {
    ...
    return this.editZone(zone, {
      ...
      successNotification: (zone) =>
        `New zone (${zone.name}) successfully created.`,
      onConfirm: () => {
        // make sure to push zone to serviceArea model
        let serviceArea = this.store.peekRecord(
          'service-area',
          zone.service_area_uuid
        );
        ...
        return zone.save().then((zone) => {
          if (typeof options.successNotification === 'function') {
            ...
            this.notifications.success(
              options.successNotification || `${zone.name} details updated.`
            );
          }
        });
      },
  ...
  createServiceArea() {
    ...
    return this.editServiceArea(serviceArea, {
      ...
      acceptButtonText: 'Confirm & Create',
      successNotification: (serviceArea) =>
        `New service area (${serviceArea.name}) created.`,
      onConfirm: () => {
        refreshRoute(this);
      },
    });
    ...
    this.modalsManager.show('modals/service-area-form', {
      ...
      confirm: (modal) => {
        ...
        return serviceArea.save().then((serviceArea) => {
          if (typeof options.successNotification === 'function') {
            this.notifications.success(
              options.successNotification(serviceArea)
            );
          } else {
            this.notifications.success(
              options.successNotification ||
                `${serviceArea.name} details updated.`
            );
          }
        });
      },
      ...
      confirm: (modal) => {
        ...
        return serviceArea.save().then((serviceArea) => {
          if (typeof options.successNotification === 'function') {
            ...
            this.notifications.success(
              options.successNotification || `${serviceArea.name} renamed.`
            );
          }
        });
      },
