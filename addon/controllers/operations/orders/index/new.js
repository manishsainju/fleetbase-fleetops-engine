...
export default class OperationsOrdersIndexNewController extends Controller {
  ...
  get isValid() {
    const { isMultipleDropoffOrder, isSubscriptionValid, isFetchingQuotes } =
      this;
  ...
  get payloadCoordinates() {
    ...
    waypoints.pushObjects([
      this.payload.pickup,
      ...this.waypoints.map((waypoint) => waypoint.place),
      this.payload.dropoff,
    ]);
    ...
    this.waypoints.forEach((waypoint) => {
      ...
      if (importId) {
        const entities = this.entities.filter(
          (entity) => entity._import_id === importId
        );
  ...
  @action createOrder() {
    ...
    const route = this.leafletOptimizedRoute
  ...
  @action importOrder() {
    ...
    this.modalsManager.show('modals/order-import', {
      ...
      confirm: async (modal) => {
        ...
        this.previewDraftOrderRoute(
          this.payload,
          this.waypoints,
          this.isMultipleDropoffOrder
        );
        modal.done();
      },
      decline: (modal) => {
  ...
  @action async toggleAdhoc(on) {
    ...
    if (on) {
      const company = this.store.peekRecord(
        'company',
        this.currentUser.companyId
      );
      this.order.adhoc_distance = company?.options?.fleetops?.adhoc_distance;
    } else {
      this.order.adhoc_distance = null;
    }
  }
      ...
      .then((serviceQuotes) => {
        ...
        if (this.serviceQuotes.length && this.isUsingIntegratedVendor) {
          set(
            ...
            this.serviceQuotes.firstObject?.uuid
          );
        }
      })
      .catch(() => {
        this.notifications.warning(
          'Failed to fetch service quotes for this order.'
        );
      })
      .finally(() => {
        this.isFetchingQuotes = false;
  ...
  @action createPlaceArrayFromPayload(
    payload,
    waypoints,
  ...
  @action previewDraftOrderRoute(
    ...
    this.routePreviewArray = this.createPlaceArrayFromPayload(
      payload,
      waypoints,
      isMultipleDropoffOrder
    );
    ...
    if (response && response.code === 'Ok') {
      ...
      if (response.waypoints && isArray(response.waypoints)) {
        ...
        for (let i = 0; i < responseWaypoints.length; i++) {
          ...
          const optimizedWaypointMarker = new L.Marker(
            extractCoordinates(optimizedWaypoint.location)
          ).addTo(leafletMap);
      ...
      // eslint-disable-next-line no-undef
      let optimizedRoute = (this.optimizedRoutePolyline = new L.Polyline(
        route,
        { color: 'red' }
      ).addTo(leafletMap));
  ...
  @action async newFacilitator() {
    const type = await this.modalsManager.userSelectOption(
